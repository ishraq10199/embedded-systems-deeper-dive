...
void unaligned_memory_access_handler(ExceptionFrame *frame) {
	printf(
		"Encountered an unaligned memory access error at PC=0x%08lX. Skipping instruction.\r\n",
		frame->pc
	);

	/* First, we need to clear the fault flag (write-1-to-clear) so it doesn't retrigger */
	SCB->CFSR = UFSR_UNALIGNED;

	/* We do the same thing we did for the divide by zero handler (i.e. skip the instruction) */
...
void HardFault_Handler_C(ExceptionFrame *frame) {
	...
	} else if (SCB->CFSR & UFSR_UNALIGNED) {

		unaligned_memory_access_handler(frame);
