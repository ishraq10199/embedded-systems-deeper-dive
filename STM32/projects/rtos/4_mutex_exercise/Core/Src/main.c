...
int main(void)
{
  ...
  /* Disable internal buffering for stdout (otherwise will only flush on newlines) */
  ...
  int delay_arg;
  printf("Delay in milliseconds: ");
  scanf("%d", &delay_arg);
  printf("\r");


  task1Handle = osThreadNew(blinkLED, (void *)&delay_arg, &task1_attributes);
