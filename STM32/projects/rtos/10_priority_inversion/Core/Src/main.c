...
const osThreadAttr_t defaultTask_attributes = { .name = "defaultTask",
		.stack_size = 128 * 4, .priority = (osPriority_t) osPriorityNormal1, };
/* USER CODE BEGIN PV */
...
const osThreadAttr_t taskL_attributes = { .name = "taskL",
		.stack_size = 512 * 4, .priority = (osPriority_t) osPriorityNormal1, };
const osThreadAttr_t taskM_attributes = { .name = "taskM",
		.stack_size = 512 * 4, .priority = (osPriority_t) osPriorityNormal2, };
const osThreadAttr_t taskH_attributes = { .name = "taskH",
		.stack_size = 512 * 4, .priority = (osPriority_t) osPriorityNormal3, };
...
void StartDefaultTask(void *argument) {
	...
	taskLHandle = osThreadNew(doTaskL, NULL, &taskL_attributes);

	// Introduce a delay to force priority inversion
	vTaskDelay(1 / portTICK_PERIOD_MS);

	// Start Task H (high priority)
	taskHHandle = osThreadNew(doTaskH, NULL, &taskH_attributes);

	// Start Task M (medium priority)
	taskMHandle = osThreadNew(doTaskM, NULL, &taskM_attributes);
